...
def random_individual(ele):
    a = random.randint(0, 1)
    if a:
        iter = random.randint(0, 10)
        for b in range(iter):
...
def fitness(individual):
    horizontal_collisions = sum(
        [individual.count(queen)-1 for queen in individual])/2
...
if __name__ == "__main__":
    ...
    lst = list(map(int, input("\nEnter the numbers : ").strip().split()))[:8]
    population = [random_individual(lst) for _ in range(100)]
    generation = 1
